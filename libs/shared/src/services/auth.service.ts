...
import { WPTokenDTO } from '@wion-fnb/account/ui';
...
export class WAuthService {
	...
	constructor(@Inject(FNBKEY_ACCESSTOKEN) private __keyBearerToken: string,
	...
	refreshToken(token: WPTokenDTO | null): Observable<TDSSafeAny> {
		...
		return this.signInService.signInRefreshToken({
		    body: {
	...
	getCacheToken(): Observable<TDSSafeAny> {
		return this.cacheHelperService.getItem(this.__keyBearerToken)
	...
	setCacheToken(token: WPTokenDTO): void {
		this.cacheHelperService.setItem(this.__keyBearerToken, token.accessToken!);
	}
	...
	getAccessToken(): WPTokenDTO | null {
		let cacheToken = localStorage.getItem(this.__keyBearerToken);
		if(cacheToken) {
